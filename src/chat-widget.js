(() => {
  ...
  function createWidgetDOM(widgetId, config) {
    ...
    <div class="header" id="${widgetId}-header">
      <h3>${title}</h3>
      ${mode === "popup" ? `<button type="button" class="close" id="${widgetId}-close">\xD7</button>` : ""}
    </div>
  ...
  var ChatWidget = class {
    constructor(scriptElement) {
      ...
      this.widgetId = scriptElement.id || "chat-widget-" + Math.random().toString(36).substr(2, 9);
      this.config = {
    ...
    sendMessage() {
      ...
      if (message) {
        ...
        this.api.sendMessage(
          message,
          (text, sender) => this.addMessage(text, sender)
        );
        this.textarea.value = "";
        this.textarea.style.height = "auto";
      }
