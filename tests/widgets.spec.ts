...
test.describe('Widget System Tests', () => {
    ...
    await page.setContent(`
      ...
      <body>
        ...
        <script>
          // Mock widget system
          window.widgetSystem = {
            createWidget: function(widgetData, widgetId) {
              ...
              if (widgetData.type === 'buttons' && widgetData.options) {
                ...
                widgetData.options.forEach(option => {
                  ...
                  button.addEventListener('click', () => {
                    ...
                    this.simulateBotResponse(option.value);
                  });
            ...
            addMessage: function(text, sender, widgetData) {
            ...
            simulateBotResponse: function(userInput) {
              ...
              setTimeout(() => {
                this.addMessage(response.text, 'bot', response.widget);
              }, 100);
            },
          ...
          document.getElementById('send').addEventListener('click', () => {
            ...
            if (message) {
              window.widgetSystem.sendMessage(message);
              textarea.value = '';
            }
          });
          ...
          window.widgetSystem.addMessage('Hello! I\'m your virtual assistant. Type \'menu\' to see interactive options or \'options\' for more choices.', 'bot');
        </script>
      </body>
      </html>
    `);
  ...
  test('should display welcome message', async ({ page }) => {
    const messagesContainer = page.locator('[id^="chat-widget-"] .messages');
  ...
  test('should show buttons widget when typing "menu"', async ({ page }) => {
    ...
    const widgetButtons = page.locator('[id^="chat-widget-"] .widget-button');
  ...
  test('should handle button clicks correctly', async ({ page }) => {
    ...
    const supportButton = allButtons.filter({ hasText: 'Customer Support' });
    ...
    // Check if support options are displayed
    const supportButtons = page.locator('[id^="chat-widget-"] .widget-button');
  ...
  test('should handle multi-level widget navigation', async ({ page }) => {
    ...
    const salesButton = allButtons1.filter({ hasText: 'Sales' });
  ...
  test('should handle regular text messages without widgets', async ({ page }) => {
    ...
    const widgets = page.locator('[id^="chat-widget-"] .widget');
  ...
  test('should handle Shift+Enter for new lines', async ({ page }) => {
    ...
    // Check if message was sent with new line
    const userMessages = page.locator('[id^="chat-widget-"] .user-message');
    await expect(userMessages.last()).toContainText('line 1\nline 2');
  });
